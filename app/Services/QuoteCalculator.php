...
class QuoteCalculator
{
    ...
    public function calculate(Quote $quote): Quote
    {
        ...
        // ── Markup (skipped for public quotes or when disabled) ─────────
        $applyMarkup = $quote->user_id !== null && $quote->apply_markup;
        ...
        if (!$applyMarkup) {
            ...
            $pluginMarkupAmount = round($pluginSubtotal * (config('quote.plugin_markup') / 100), 2);
        ...
        $vat         = $quote->apply_vat ? round($totalExVat * config('quote.vat_rate'), 2) : 0;
    ...
    {
        foreach (config('quote.markup_tiers') as $threshold => $rate) {
            if ($amount >= $threshold) {
                return $rate;
            }
        }
